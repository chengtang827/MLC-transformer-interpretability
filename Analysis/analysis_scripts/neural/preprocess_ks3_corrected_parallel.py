...
def raster_to_bins(path_internal, path_external, date_str, rewrite=0):
    ...
    def _raster_to_bin(idx_unit):
        save_name = save_dir / (str(idx_unit) + '.obj')
        ...
        for t in range(n_trials):
            ...
            idx_sac_start = 0
            if has_cue2:
                raster_cue2_t = raster_cue2_on[idx_unit, t]
                idx_cue2_start = 0
...
if __name__ == '__main__':
    NP_dates = ['20230917_F_g0']
    base_dir = Path('/Users/laptopd/Documents/Compositionality/Analysis/spikedata/F/NP')
    session_dates = ['20230508']

    n_sessions = len(session_dates)

    path_internal = base_dir / '20230508_F_g0_0/kilosort3_corrected_output/iteration1/sorter_output'
    path_external = Path('/Volumes/Expansion/Cheng/processed data') / '20230508_F_g0_0/phy'
    if not os.path.exists(path_external):
        os.mkdir(path_external)

    date_str = session_dates[0]
