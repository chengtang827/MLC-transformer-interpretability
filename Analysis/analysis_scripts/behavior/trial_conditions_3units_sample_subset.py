...
start_moves_down_sample = []
for sm in start_moves:
    ...
    if start_t[0] % 2 == 1:
        ...
        if sum(move_t) % 2 == 0:
            continue
    start_moves_down_sample.append(sm.tolist())
...
down_sample_trained = []
down_sample_untrained = []
for i in range(len(start_moves_down_sample)):
    ds = start_moves_down_sample[i]
    if (ds in target_set1) or (ds in target_set2):
        down_sample_trained.append(i)
    else:
        down_sample_untrained.append(i)

target_set4 = copy.deepcopy(start_moves_down_sample) # systematic test of all conditions

p1 = []
for i0 in range(len(target_set4)):
    if i0 in down_sample_trained:
        p1.append(5)
    elif i0 in down_sample_untrained:
        p1.append(1)
