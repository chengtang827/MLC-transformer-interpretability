...
def _get_trial_type(session):
    """

    :param session:
    :return:
    """
    idx_trained_trials = np.zeros(session.n_total)
    for i in range(session.n_total):
        target_xy = session.target_xy[i, :]
        idx_target = [j for j in range(len(session.training_set)) if (target_xy == session.training_set[j]).all()]
        if len(idx_target) == 1 and session.training_set_order[idx_target[0]] == session.order_xy[i]:
            idx_trained_trials[i] = 1

    return idx_trained_trials == 1
...
def _prior_distribution(session_dates):
    ...
    for s in range(n_session):
        ...
        flag_probe = _get_probe_trials(session)
...
def _training_progress_confusion_gen_only(session_dates):
    n_session = len(session_dates)
    x_range = 13
    y_range = 13  # 8+1+8
    speed = 2
    ...
    for i in range(target_all.shape[0]):
        tmp = saccade_all[i, :] - target_all[i, :]

        confusion_one[int(tmp[0] + (n_lines - 1) / 2), int(tmp[1] + (n_lines - 1) / 2)] += 1
    ...
    # for s in range(n_session):
    #     session = _get_session(sessions, session_dates[s])
    #     flag_invis = _get_probe_trials(session)
    #
    #     target_set = np.array(session.target_set)
    #
    ...
    #     # trials where there is response
    #     idx_val_trials = np.where(~np.isnan(session.reported_xy[:, 0].astype(float)))[0].astype(int)
    #     target_ids = session.target_id[idx_val_trials]
    #     saccade_xy = session.reported_xy[idx_val_trials]
    #     # target_xy_val = session.target_xy[idx_val_trials]
    #
    #     flag_invis = flag_invis[idx_val_trials]
    #     flag_trained = flag_trained[idx_val_trials]
    #
    #     idx_vis = np.where(flag_invis == 0)[0]
    #     idx_invis = np.where(flag_invis == 1)[0]
    #     idx_invis_train = np.where(np.logical_and(flag_invis, flag_trained) == 1)[0]
    #
    #     trial_type_list = [idx_vis, idx_invis, idx_invis_train]
    #     for i0, idx_by_trial_type in enumerate(trial_type_list):
    #         start_xy_i0 = target_set[target_ids, 0, :][idx_by_trial_type, :]
    #         vector_xy_i0 = target_set[target_ids, 1, :][idx_by_trial_type, :]
    #         target_xy_i0 = start_xy_i0 + vector_xy_i0
    #         saccade_xy_i0 = saccade_xy[idx_by_trial_type, :]
    #         error[s, i0] = np.sum(np.sum((target_xy_i0 - saccade_xy_i0) ** 2, 1) ** 0.5) / idx_by_trial_type.shape[0]
    #
    #     start_xy = target_set[target_ids, 0, :]
    #     vector_xy = target_set[target_ids, 1, :]
    #     target_xy = start_xy + vector_xy
    #
    #     # the last session
    #     if s == (n_session - 1):
    #         error_s = np.zeros(len(flag_invis))
    #         idx_probe_trials_s = flag_invis
    #
    #     for i0 in range(len(idx_val_trials)):  # visible/probe
    #         x = target_xy[i0, 0]
    #         y = target_xy[i0, 1]
...
def _confusion_each_generalization_only(session_dates):
    n_session = len(session_dates)
    start_all = np.zeros((0, 2))
    target_all = np.zeros((0, 2))
    saccade_all = np.zeros((0, 2))
...
def training_progress_confusion(session_dates, sessions):
    n_session = len(session_dates)
    x_range = 13
    y_range = 13  # 8+1+8
    speed = 2
...
if __name__ == "__main__":
    session_file = open('/sessions_F.obj', 'rb')
    sessions = pickle.load(session_file)
    session_file.close()

    session_sample_all_dates = ['20231128', '20231129', '20231130', '20231201', '20231204', '20231205', '20231206',
                                '20231207', '20231210', '20231211']
