...
def main(session_dir, iterations):
    ...
    for i0 in range(iterations):
        ...
        peaks = detect_peaks(recording=preprocessed, method="locally_exclusive", gather_mode='npy',
                             folder=peak_base_folder / ('iteration' + str(int(i0))) / 'tmp_before',
                             names=['peaks_monopolar_triangulation'],
                             detect_threshold=9, peak_sign='neg',
                             noise_levels=noise_levels, **job_kwargs)
        t2 = time.time()
        print('t2: ' + str(t2 - t1))
        some_peaks = select_peaks(peaks, method='smart_sampling_amplitudes', noise_levels=noise_levels, n_peaks=900000)
        # some_peaks = peaks
        t3 = time.time()
        print('t3: ' + str(t3 - t2))
        ...
        motion, temporal_bins, spatial_bins = estimate_motion(recording=preprocessed,
                                                              ...
                                                              progress_bar=True, ** motion_kwargs)
