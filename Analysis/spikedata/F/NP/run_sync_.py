...
for s in np.arange(1):
    # path = date_str + '/ks_3_output_pre_v6_' + str(s) + '/'
    path = date_str + '/ks_3_output_pre_v6/'

    path_ = date_str + '/ks_3_output_pre_v6/'
    raster_path = date_str + '/raster_' + str(s)
    if not os.path.exists(raster_path):
        os.mkdir(raster_path)

    nidaq_meta = _get_nidaq_meta(path_)
    ...
    imec_meta = _get_imec_meta(path_)
    ...
    reader = _get_nidaq_reader(path_)
    ...
    good_cluster_id = np.where(group == 'good')[0]
    ...
    n_good_units = 100
    spiketimes_all_units = np.ndarray(n_good_units, dtype=object)

    for i in range(n_good_units):
        spiketimes_all_units[i] = spiketimes[spike_clusters == good_cluster_id[i]]
    ...
    raster = np.ndarray((n_good_units, session.n_total), dtype=object)
    before = -2
    after = 6
    for i in range(n_good_units):
    ...
    psth_cue = np.zeros((n_good_units, session.n_total, bins.shape[1]))
    psth_sac = np.zeros((n_good_units, session.n_total, bins.shape[1]))
    for i in range(n_good_units):
        ...
        raster_name_i = raster_path + '/' + str(good_cluster_id[i]) + '_' + group[good_cluster_id[i]] + '_raster' + '.pdf'
        ...
        ax[0].set_title('Cue onset')
        ax[1].set_title('Saccade onset')
        psth_name_i = raster_path + '/' + str(good_cluster_id[i]) + '_' + group[good_cluster_id[i]] + '_psth' + '.pdf'
