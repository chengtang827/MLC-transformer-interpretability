...
def _cleanEntry(data, idx_begin):
    ...
    data = np.asarray(data, dtype=object)[idx_clean]
    return data
...
def _parse_n_save(filename, dir_raw, save_dir, exclude_file):
    session = Session()
    session.date = filename.split('-')[2]
    save_path = save_dir / filename
    savename = save_path.parent / (save_path.name+'.obj')
    if os.path.exists(savename):
        return

    f = MWKFile(str(dir_raw / filename))
    ...
    try:
        ...
        # # contaminated by red screen, filter out
        # cue1_times = []
        # [cue1_times.append(t.time) for t in total]
        # cue1_times = np.array(cue1_times)
        #
        #
        # idx_p = 0
        # idx_c = 0
        ...
        # while True:
        #     try:
        #         p_time = photodiode_times[idx_p]
        #         c_time = cue1_times[idx_c]
        #         if abs(p_time-c_time)<1e6:
        ...
        #         break
        #
        # photodiode_times = photodiode_times[idx_true_photo]
    except:
        photodiode_times = []
    ...
    try:
        #ball_alpha_all = f.get_events(codes=['ball_alpha'])
    ...
    for i in range(len(total) - 1):  # discard the last trial
        ...
        # idx_ball_alpha = _findLastTimestep(ball_alpha_all, idx_ball_alpha, cue1_start)
    ...
    session.eye_xy = np.array(session.eye_xy)
    ...
    pickle.dump(session, file_to_save)
    file_to_save.close()

    f = open(exclude_file, 'a+')
    f.write(filename + '\n')
    f.close()


def _merge_sessions(sessions, i):
    session_i = sessions[i]
    sessions[i - 1].merge(session_i)
    sessions = np.delete(sessions, i)
    return sessions


if __name__ == "__main__":
    dir_raw = Path('./trials')  #./trials/F/'
    dir_parsed = Path('./trials_parsed') #/F/'
    if not os.path.isdir(dir_parsed):
        os.makedirs(dir_parsed)

    exclude_file = './exclude_sessions.txt'
    subjects = ['F', 'N']
    for sub in subjects:
        filenames = os.listdir(dir_raw / sub)

        for i in range(len(filenames)):
            if filenames[i] == '.DS_Store':
                continue

            _parse_n_save(filenames[i], dir_raw / sub, dir_parsed / sub , exclude_file)
            print(filenames[i])

        # merge the sessions into a single object sessions.obj
        filenames = os.listdir(dir_parsed / sub)
        sessions = []
        dates = []
        for i in range(len(filenames)):
            if filenames[i] == '.DS_Store':
            ...
            file_i = filenames[i]
            try:
                dates.append(file_i.split('-')[2])
            except:
                a = 1

            file_to_load = open(dir_parsed / (sub + '/' + file_i), 'rb')
            sessions.append(pickle.load(file_to_load))
            file_to_load.close()

        checksum = 0
        for i in range(len(dates)):
            checksum += sessions[i].checksum
        ...
        while i < len(sessions):
            name_i = dates_[i]
            if name_i == name_pre:
                sessions = _merge_sessions(sessions, i)
                dates_ = np.delete(dates_, i)
                # i += 1
            else:
                name_pre = name_i
                i += 1
        ...
        save_path = Path('./sessions_'+sub+'.obj')#'./sessions'
        file_to_save = open(save_path, 'wb+')
        pickle.dump(sessions, file_to_save)
        file_to_save.close()
        a = 1
