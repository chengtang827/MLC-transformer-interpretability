...
def _parse_session_from_mwk2(filename, dir_raw, save_dir, exclude_file):
    session = Session()
    ...
    # write to exclude files so won't download again
...
if __name__ == "__main__":
    ...
    for sub in subjects:
        ...
        session_names_parsed = os.listdir(dir_parsed / sub)
        [session_names_parsed.remove(f) for f in session_names_parsed if not f.endswith('obj')]
        dates = np.array([name.split('-')[2] for name in session_names_parsed])
        idx_sorted_dates = dates.argsort()
        sessions = []
        for idx_date in idx_sorted_dates:
            session_name_i = session_names_parsed[idx_date]
            file_to_load = open(dir_parsed / f"{sub}/{session_name_i}", 'rb')
