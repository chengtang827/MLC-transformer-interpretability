...
import math
import time
from hooked_model import HookedBIML, HookedBIMLDecoderOnly, describe_model
from eval_org import evaluate_ll, evaluate_acc
    ...
    for epoch in range(epoch_start,nepochs+1):
        ...
        for batch_idx, train_batch in enumerate(train_dataloader):
            ...
            if step in [1,25] or step % 100 == 0:
                ...
                sys.stdout.write('{:s} ({:d} {:.0f}% finished) LR: {:.7f}, TrainLoss: {:.4f}. \n'.format(timeSince(start, float(step) / float(nsteps_estimate)),
                                         step, float(step) / float(nsteps_estimate) * 100., mylr, avg_train_loss/counter))
                sys.stdout.flush()
                ...
                val_result = evaluate_acc(val_dataloader, net, langs, max_length=10)
                mean_acc_novel = val_result['mean_acc_novel']
                print('Val Acc: {:.4f}'.format(mean_acc_novel))
                mytracker['val_acc'] = mean_acc_novel
