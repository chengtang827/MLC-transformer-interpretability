...
def get_dataset(episode_type, **kwargs):
# --------------------------------------------
...
# --------------------------------------------
    ...
    if episode_type == 'my_test': # make a dataset for each txt file in the directory
        ...
        D_train = DataAlg('train',mydir='data_algebraic', p_noise=0.01, min_ns=3)
        D_val = DataAlg('val',mydir='data_algebraic')
    elif episode_type == 'algebraic+biases': # BIML training
        D_train = DataAlgAndBias('train',mydir='data_algebraic',p_noise=0.01, min_ns=3)
        D_val = DataAlg('val',mydir='data_algebraic')
...
class Lang:
    ...
    def tensor_to_symbols(self, v, break_ = True):
...
def bundle_biml_episode(x_support,y_support,x_query,y_query,myhash,aux={}):
    ...
    #print('bundle biml content', x_support, y_support, x_query, y_query, aux)
    ns = len(x_support)
    xy_support = [ITEM_SEP]
    for j in range(ns):
...
def build_padded_tensor(list_seq, lang, add_eos=True, add_sos=False):
    ...
    max_len = max(z_lengths) # maximum length in this episodecd
...
class DataAlg(Dataset):
    ...
    def __init__(self, mode, mydir, input_symbols = None, output_symbols = None, p_noise=0., inc_support_in_query=True, min_ns=0):
...
class DataAlgSingleEpisode(DataAlg):
    # Created by Cheng Tang for testing purposes

    def __init__(self, mode, episode_path, input_symbols = None, output_symbols = None, p_noise=0., inc_support_in_query=True, min_ns=0):
        # Each episode has different latent (algebraic) grammar. 
        #  The number of support items picked uniformly from min_ns...max_ns
        #
        # Input
        ...
        self.train = mode == 'train'
        self.p_noise = p_noise
        self.list_items = [str(episode_path)] # all episode files
        if input_symbols == None:
            self.input_symbols = input_symbols_list_default
        else:
            self.input_symbols = input_symbols
