...
class PositionalEncoding(HookedRootModule):
    #
...
class HookedBIML(HookedRootModule):
    ...
    def __init__(self, hidden_size: int, input_size: int, output_size: int, PAD_idx_input: int, PAD_idx_output: int,
        ...
        #   
        super(HookedBIML, self).__init__()
        ...
        self.transformer = hooked_transformer.HookedTransformer(d_model=hidden_size, nhead=nhead, num_encoder_layers=nlayers_encoder, num_decoder_layers=nlayers_decoder,
        ...
        self.pos_embedding = PositionalEncoding(emb_size=hidden_size, dropout=dropout_p)
        self.output_embedding = nn.Embedding(output_size, hidden_size)        
        self.out = nn.Linear(hidden_size,output_size)

        self.input_embedding_hook = HookPoint()
        self.output_embedding_hook = HookPoint()
    ...
    def prep_encode(self, xq_context_padded):
        ...
        xq_context_embed = self.input_embedding_hook(
            self.input_embedding(xq_context_padded) # batch_size x maxlen_src x emb_size
            ) 

        # Add positional encoding to input embeddings
        src_embed = self.pos_embedding(xq_context_embed.transpose(0,1))
    ...
    def prep_decode(self, z_padded):
        ...
        z_embed = self.output_embedding_hook(
            self.output_embedding(z_padded) # batch_size x maxlen_tgt x emb_size
            ) 

        # Add positional encoding to target embeddings
        tgt_embed = self.pos_embedding(z_embed.transpose(0,1))
...
class HookedBIMLDecoderOnly(HookedRootModule):
    #
    # Transformer trained for meta seq2seq learning
    #
    def __init__(self, hidden_size: int, input_size: int, output_size: int, PAD_idx_input: int, PAD_idx_output: int,
        nlayers_decoder: int=6, nhead: int=8,
        dropout_p: float=0.1, ff_mult: int=4, activation='gelu'):
        #
        # Input        
        #  hidden_size : embedding size
        ...
        self.transformer = hooked_transformer.HookedTransformerDecoderOnly(d_model=hidden_size, nhead=nhead, num_decoder_layers=nlayers_decoder,
            dim_feedforward=self.dim_feedforward, dropout=dropout_p, batch_first=True, activation=activation)
