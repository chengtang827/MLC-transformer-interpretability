...
class Problem:
    ...
    def back_track(self, n_track=2, threshold=0.5, prune_names=None, metric='inner'):
        """
        ...
        assert metric in ['inner', 'magnitude']
        ...
        # target_vectors = self.target_vectors
        source_scores = []
        n_batch = source_activations[0].shape[0]
        ...
        for source_activation in source_activations:
            ...
            if metric=='inner':
                ...
                mag = np.linalg.norm(source_activation, ord=2, axis=-1)

                if self.mode=='k' or self.mode=='v':
                    mag = np.tile(mag[:, np.newaxis,:], (1, weight_mask.shape[1], 1))
                else: # 'q' or 'out'
                    mag = np.tile(mag[:,:, np.newaxis], (1, 1, weight_mask.shape[2]))
                mag_masked = mag*weight_mask
                score = mag_masked.mean()
                a=1
    ...
    def back_track(self, pred_arg, prune_names=None, plot_score=1, rewrite=False):
        ...
        while problem_stack:
            ...
            source_nodes, source_scores, arg_top_nodes, arg_q = current_problem.back_track(n_track=n_track, threshold=threshold,
                                                                                           prune_names=prune_names, metric='magnitude')
