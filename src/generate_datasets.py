...
from MLC_datasets import Lang, flip, make_hashable, bundle_biml_episode, input_symbols_list_default, output_symbols_list_default#, equiv_to_miniscan
...
def generate_rules_dataset(nsamp_train,nsamp_val,mydir='data_algebraic',episode_type='algebraic', rules = None, max_recursion = 50, max_depth = None,  nsupport = (14,14), query = (10,10), simple = False):
    ...
    os.mkdir(mydir+'/val')
    generate_episode_train, _ = get_episode_generator(episode_type, rules = rules, max_recursion = max_recursion, max_depth = max_depth, nsupport = nsupport, query = query, simple = simple)
...
def get_episode_generator(episode_type, rules = None, max_recursion = 50, max_depth = None, nsupport = (14,14), query = (10,10), simple = False):
    ...
    if episode_type == 'algebraic':
        nsupport = random.randint(nsupport[0], nsupport[1])
        query = random.randint(query[0], query[1])

        generate_episode_train = lambda tabu_episodes : generate_rules_episode(nsupport=nsupport,nquery=query,nprims=4,nrules=3,
                tabu_list=tabu_episodes,maxlen=8, rules = rules, max_recursion = max_recursion, max_depth = max_depth, simple = simple)
...
def generate_rules_episode(nsupport,nquery,nprims,nrules,maxlen,tabu_list=[],max_try_novel=100, rules = None, max_recursion = 50, max_depth = None, simple = False):
    ...
    while True:
        if rules is not None:
            nrules = min(len(input_symbols) - nprims, len(rules))
            rules_subset = random.sample(rules, nrules)
            G = generate_grammar(nprims,rules,input_symbols,output_symbols,max_len=maxlen, max_recursion = max_recursion)
        else:
            G = generate_random_rules(nprims,nrules,input_symbols,output_symbols,max_len=maxlen) #modify to input your own grammar


        myhash = make_hashable(G)

        if simple:
            D_s = sample_examples(nsupport, G, maxlen_input=maxlen,maxlen_output=maxlen, max_depth = max_depth)
            D_q = sample_examples(nquery, G, maxlen_input=maxlen,maxlen_output=maxlen, max_depth = max_depth, noprim = True)
            np.random.shuffle(D_s)
            np.random.shuffle(D_q)

            x_support = [d[0].split(' ') for d in D_s]
            y_support = [d[1].split(' ') for d in D_s]
            x_query = [d[0].split(' ') for d in D_q]
            y_query = [d[1].split(' ') for d in D_q]

        else:
            D = sample_examples(ntotal,G,maxlen_input=maxlen,maxlen_output=maxlen, max_depth = max_depth)
            np.random.shuffle(D)
        ...
        #equiv_to_minscan no longer exists, so just removing this because it's not necessary for our purposes anyways
...
def generate_grammar(nprims, rules, input_symbols, output_symbols, max_len, max_recursion = 50):
    #ensure len(rules)<nprims
    ...
    rules_new, input_symbol_options = generate_prims(nprims, input_symbol_options, output_symbol_options)
    for i, rule in enumerate(rules):
        input_symbol = input_symbol_options[i]
        LHS = re.sub('func', input_symbol, rule[0])
        RHS = rule[1]
        rules_new.append(Rule(LHS, RHS))
    # rules_new.append(icon_concat_rule)
    return Grammar(rules_new, input_symbols, max_recursion = max_recursion)
...
def sample_examples(nexamples,G,maxlen_input,maxlen_output,maxntry=10000, max_depth = None, noprim=False):
    # Input
    ...
    while len(D)<nexamples:
        dat_in = sample_from_pcfg(CFG, maxlen_input, max_depth = max_depth, noprim = noprim)
...
def sample_from_pcfg(CFG,maxlen, max_depth=None, noprim = False): 
    ...
    while True: 
        ...
        if len(list_expand) > maxlen:
            return ''
        if max_depth is not None and x_count>(max_depth):
            #print(mystr, x_count, max_depth)
            return ''
...
if __name__ == "__main__":
    ...
    rules = [
            ['x1 func x2', '[x2] [x1] [x2] [x1] [x1]'],
            ['x1 func x2', '[x2] [x1] [x1] [x2] [x2]'],
            ['x1 func x2', '[x1] [x2] [x1] [x2] [x1]'],
            ['x1 func x2', '[x1] [x2] [x2] [x1] [x2]'],
            ]

    seed_all()
    generate_rules_dataset(200,200,mydir='test_data_long',episode_type='algebraic', rules=rules,  max_depth = 1, nsupport = (14,14), query = (10,10), simple = True)
