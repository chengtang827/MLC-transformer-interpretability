...
if __name__ == "__main__":
        ...
        if 0:
            ...
            perturb_analyzer.perturb_dec_cross_1_5_k_for_logit(block = 'dec',layer = 1, type = 'cross', head = 5)
        ...
        if 0: # plot attention weights
            # dataset_path = project_dir / 'test_data_long' / 'single_trial'
            utils.plot_attention_patterns(dataset=dataset, net=net, save_dir=plot_dir, rewrite=1)     

        if 0: # plot backtrack graph
            # circuit = backtrack_analyzer.back_track(arg_output=[0,1], plot_score=1, metric='VAF', sequential=1, n_track=1, threshold=0.3, rewrite=1)
            circuit = backtrack_analyzer.back_track(arg_output=[0,1], plot_score=1, metric='loss', 
                                                    chained=1, n_track=1, threshold=0.3, keep_one_only=0)


        backtrack_analyzer.backtrack_dec_cross_1_5_qk(block = 'dec',layer = 1, type = 'cross', head = 5, rewrite=0)
